...
# ----------------- Contest Attempt (per student) -----------------
class ContestAttempt(models.Model):
    student = models.ForeignKey(Student, on_delete=models.CASCADE, related_name="attempts")
    # Assuming single active contest; keep nullable for now to avoid strict coupling
    contest = models.ForeignKey(Contest, on_delete=models.SET_NULL, null=True, blank=True, related_name="attempts")
    start_at = models.DateTimeField(default=timezone.now)
    duration_minutes = models.PositiveIntegerField(default=60)
    end_at = models.DateTimeField()
    ended_at = models.DateTimeField(null=True, blank=True)
    ended_reason = models.CharField(max_length=20, null=True, blank=True)  # 'manual' | 'timeout'
    is_locked = models.BooleanField(default=False)

    class Meta:
        indexes = [
            models.Index(fields=["student", "end_at"]),
        ]

    def save(self, *args, **kwargs):
        if not self.end_at:
            self.end_at = (self.start_at or timezone.now()) + timezone.timedelta(minutes=self.duration_minutes or 60)
...
class Problem(models.Model):
    ...
    function_params = JSONField(
        default=list, blank=True, help_text="List of parameter names (e.g., ['a', 'b'])"
    )
...
def testcase_upload_path(instance, filename):
    ...
    """
    return os.path.join(
        "testcases", f"problem_{instance.problem.id}", f"{instance.language}_{filename}"
    )
...
class UserSolution(models.Model):
    ...
    # Fastest accepted execution time across all AC submissions for this problem (sum of test case times in ms)
    best_time_ms = models.FloatField(null=True, blank=True)
    # Optional link to the submission that achieved the best time
    # Kept non-relational to avoid migration ordering issues across apps; store UUID as text
    best_submission_id = models.UUIDField(null=True, blank=True)
...
class JuniorSubmission(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    orig_submission = models.UUIDField(null=False, blank=False)
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="junior_submissions", null=True, blank=True
    )
    problem = models.ForeignKey(Problem, on_delete=models.CASCADE, related_name="junior_submissions")
    code = models.TextField()
    language = models.CharField(max_length=20, default="python")
    status = models.CharField(max_length=10, choices=Submission.Status.choices, default=Submission.Status.QUEUED)
    score = models.FloatField(default=0)
    max_score = models.FloatField(default=0)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"Junior {self.orig_submission} - {self.status} ({self.score}/{self.max_score})"


class SeniorSubmission(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    orig_submission = models.UUIDField(null=False, blank=False)
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="senior_submissions", null=True, blank=True
    )
    problem = models.ForeignKey(Problem, on_delete=models.CASCADE, related_name="senior_submissions")
    code = models.TextField()
    language = models.CharField(max_length=20, default="python")
    status = models.CharField(max_length=10, choices=Submission.Status.choices, default=Submission.Status.QUEUED)
    score = models.FloatField(default=0)
