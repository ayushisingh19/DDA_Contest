...
class SubmissionWrapper:
    ...
    def load_template(self, language: str) -> str:
        ...
        template_file = os.path.join(
            self.problem_path, f"solution.{lang_config['file_ext']}"
        )
    ...
    def wrap_python_code(self, user_code: str, template: str) -> str:
        ...
        wrapped_code = re.sub(
            template_pattern, user_solution, template, flags=re.DOTALL
        )
        ...
        try:
            ...
            for i, test_case in enumerate(test_cases):
                ...
                effective_memory = max(
                    ...
                    execution_result["user_memory"] - baseline_memory,
                )
                ...
                test_result = {
                    ...
                    "raw_time": execution_result[
                        "user_time"
                    ],  # Raw execution time from Judge0
                    "raw_memory": execution_result[
                        "user_memory"
                    ],  # Raw memory usage from Judge0
                ...
                summary["max_effective_time"] = max(
                    summary["max_effective_time"], effective_time
                )
                summary["total_effective_memory"] += effective_memory
                summary["max_effective_memory"] = max(
                    summary["max_effective_memory"], effective_memory
                )

                # Stop on first error if requested
                if stop_on_first_error and (
                    execution_result["status"] in ["CE", "TLE", "MLE", "RE"]
                    or (execution_result["status"] == "AC" and not is_correct)
                ):
                    print(
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python submission_wrapper.py <problem_name> <language_or_folder> [user_code_file] [options]"
        )
        print("Options:")
        print(
            "  Single file: python submission_wrapper.py merge_k_lists python user_solution.py"
        )
        print(
            "  All files:   python submission_wrapper.py merge_k_lists submissions/merge_k_lists/"
        )
        print("  --max-cases N         Run only first N test cases")
        print(
            "  --continue-on-error   Continue testing all cases even after failures (default: stop on first error)"
        )
        print(
            "Note: By default, evaluation stops on first error since scoring formula requires 100% correctness."
        )
        sys.exit(1)
    ...
    if os.path.isdir(second_arg):
        ...
        for i, (file_path, language, filename) in enumerate(solution_files, 1):
            ...
            results = wrapper.evaluate_submission(
                user_code, language, max_cases, stop_on_error
            )
            ...
            if summary["passed"] == total_tested:
                print(
                    f"   🎉 All tests passed! Max time: {summary['max_effective_time']:.6f}s"
                )
            else:
                print(
                    f"   ❌ Failed: {summary['failed']}, TLE: {summary['tle']}, MLE: {summary['mle']}, CE: {summary['ce']}"
                )
        ...
        if all_results:
            ...
            for i, result in enumerate(all_results, 1):
                ...
                print(
                    f"{i:<4} {result['filename']:<30} {summary['passed']}/{total_cases:<8} {max_time:<12.6f} {status}"
                )
            ...
            print(
                "📄 Individual detailed results saved as separate files for each submission"
            )
        ...
        if len(sys.argv) < 4:
            print(
                "Error: For single file mode, you need: python submission_wrapper.py <problem_name> <language> <user_code_file>"
            )
            sys.exit(1)
        ...
        visible_cases = [
            result for result in results["test_cases"] if result.get("expected_output")
        ]
        if visible_cases:
            print("\n📋 Visible Test Cases:")
            for result in visible_cases:
                status_emoji = (
                    "✅" if result["is_correct"] and result["status"] == "AC" else "❌"
                )
                print(
                    f"   {status_emoji} Case {result['test_case_no']}: {result['status']} "
        ...
        output_file = (
            f"user_submission_{problem_name}_{language}_{int(time.time())}.json"
        )
